...
@jit(nopython=True)
def LBMASH(sd, dt, j, gam, States):
    # decay States[0] to States[1]
    Z0 = 1.0*sd.c[States[0]]
    Z1 = 1.0*sd.c[States[1]]
    dr1T_ = 1.0*sd.dr1T[States[1]]
    Z1T_ = 1.0*sd.Z1T[States[1]]

    # Total population of both states
    pop0 = np.abs(Z0*np.conj(Z0))
    pop1 = np.abs(Z1*np.conj(Z1))

    Z0P = np.sqrt(np.exp(-gam*dt)*pop0+(1-np.exp(-gam*dt))*m.ZPE)

    dP1 = (1-np.exp(-gam*dt))*(pop0-m.ZPE)
    if(pop1+dP1<0):
        Z1T_ += dP1
        Z1P = np.abs(Z1)
    elif(Z1T_<0):
        ...
        if(Z1T_+pop1<=0):
            ...
            Z1P = np.sqrt(pop1+Z1T_)
            Z1T_ = 0
    else:
        Z1P = np.sqrt(pop1+dP1)

    # Determine off-diagonal decay rate
    dr0 = np.exp(-gam*dt/2)*np.abs(Z0)/Z0P
    dr1 = np.abs(Z1)/Z1P
    ...
    if(sd.sample):
        if(dr0<1.0): # use regular interpolation
            drnum = dr0*1000000
            dTheta0 = LID[int(np.floor(drnum)),1]*(1+np.floor(drnum)-drnum) + LID[int(np.floor(drnum))+1,1]*(drnum-np.floor(drnum))
        else:
            dTheta0 = 0.0
            
        if(dr1<1.0): # use regular interpolation
            drnum = dr1*1000000
            dTheta1 = LID[int(np.floor(drnum)),1]*(1+np.floor(drnum)-drnum) + LID[int(np.floor(drnum))+1,1]*(drnum-np.floor(drnum))
        else:
            dTheta1 = 0.0
        ...
        Theta0 = sd.randLB[j,0] * dt/m.dtF
        Theta1 = sd.randLB[j,1] * dt/m.dtF

    Z0P = np.exp(1j*(Theta0+np.angle(Z0)))*Z0P
    Z1P = np.exp(1j*(Theta1+np.angle(Z1)))*Z1P
    sd.c[States[0]] = Z0P
    sd.c[States[1]] = Z1P
    sd.dr1T[States[1]] = dr1T_
    sd.Z1T[States[1]] = Z1T_
...
def PropL(sd, dt):
    if(m.LBasis >= 0):
        LBasisChange(sd)
        LRates = getLRates(sd)
        LStates = getLStates(sd)
        for j in range(len(LRates)):
            LBMASH(sd, dt, j, LRates[j], [LStates[j,0],LStates[j,1]])
        LBasisChangeRevert(sd)
        
@jit(nopython=True)
def PropHL(sd0, sd1, dt):
    PropH(sd0, sd1, dt)
    PropL(sd1, dt)
...
@jit(nopython=True)
def doRescale(sd, hd):
    return not np.any(m.dontRescaleStates == sd.acst or m.dontRescaleStates == hd.acst_att)
...
def hop(sd, hd): # attempt a hop
    ...
    if(doRescale(sd, hd)):
        ...
        hd.accepted = True # auto accept if acst or acst_att is in dontRescaleStates
        sd.P[:] = sd.P*np.sqrt(sd.M)
        
@jit(nopython=True)
def hop_test(sd, hd): # test a hop to see if accepted and rescale_mag; dont change P
    hd.rescale_mag = 1.0
    sd.P[:] = sd.P/np.sqrt(sd.M) # mass-scaled momentum
    if(doRescale(sd, hd)):
        potdiff = np.real(sd.E[hd.acst_att] - sd.E[sd.acst])
        ...
        P_proj_norm = np.sqrt(np.dot(P_proj,P_proj))
        if(P_proj_norm**2 < 2*potdiff): # rejected hop
            hd.accepted_test = False
            sd.P[:] = sd.P*np.sqrt(sd.M)
        else: # accepted hop
            hd.rescale_mag = np.sqrt(P_proj_norm**2 - 2*potdiff)/P_proj_norm
            hd.accepted_test = True
            sd.P[:] = sd.P*np.sqrt(sd.M)
    else:
        hd.accepted_test = True # auto accept if acst or acst_att is in dontRescaleStates
        sd.P[:] = sd.P*np.sqrt(sd.M)
        
@jit(nopython=True) # reverse momenta along hop_dir regardless of energy
def reverseP(sd, hd): # attempt a hop (dont check if a==b)
    hd.rescale_mag = 1.0
    sd.P[:] = sd.P/np.sqrt(sd.M) # mass-scaled momentum
    if(doRescale(sd, hd)):
        hop_dir = getHopDir(sd, hd)
...
@jit(nopython=True)   
def hop_setup(sd0, sd1, sdH, sdL, hd): # determine rescaling conditions and perform hop attempt
    print("Hop attempt!")
    getACST_att(sd1, hd)
    dt_test = 1.0*m.dtLB
    sdH.copy(sd1)
    PropH(sd1, sdH, dt_test)
    sdL.copy(sdH)
    PropL(sdL, dt_test)
    sd1.dP = pop_diff(sd1)
    sdH.dP = pop_diff(sdH)
    sdL.dP = pop_diff(sdL)
    d_dt_H = (sdH.dP - sd1.dP)/dt_test
    d_dt_L = (sdL.dP - sdH.dP)/dt_test
    hop_test(sd1, hd)
    hd.rescale = False
    hd.reverse = False
    if(hd.accepted_test):
        hd.accepted = True
        if(d_dt_L < -d_dt_H*hd.rescale_mag and doRescale(sd1, hd)):
            hd.rescale = True
        elif(d_dt_L > d_dt_H and doRescale(sd1, hd)):
            hd.accepted = False
            hd.reverse = True
    else:
        if(d_dt_L > d_dt_H and doRescale(sd1, hd)):
            hd.accepted = False
            hd.reverse = True
        else:
            hd.accepted = True
...
def fullStep(sd0, sd1, sdH, sdL, hd):
    sd0.randLB = np.zeros((m.NJump,2))
    sd0.sample = True
    sd1.copy(sd0)
    hd.dt = 1.0*m.dtF
...
def runTraj():
    ...
    for itraj in range(m.NTraj): # repeat simulation for each trajectory
        ...
        sd0.M[:] = m.M
        sd0.dr1T[:] = np.ones(m.NStates) # keeps track of over-coherence when necessary
        sd0.Z1T[:] = np.zeros(m.NStates) # keeps track of overly-negative population when necessary
